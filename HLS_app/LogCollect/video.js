...
function setstudentNumber2(hashNumber) {
  console.log("aaaaa");
  console.log("aaaaa" + hashNumber);

  data.StudentNumber = hashNumber;
  actionData.StudentNumber = hashNumber;
  setUserAgent();
}
...
function actionDataUpdateAndSend(Action) {
  //actionDataをアップデートしてビーコン作成しログ送信
...
function createBecon(dataForSend) {
  console.log("HlsData:" + dataForSend);
  if (localStorage.getItem("isAgree") == "0") {
...
window.setTimeout(function () {
  console.log("set時間経過");
  if (localStorage.getItem("isAgree") == "0" && s == 1) {
    fInitFingerprintJS();
  }
}, 60000);
